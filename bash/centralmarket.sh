...
alias cm-switch-branch='cd ~/dev/central-market/wordpress/centralmarket.com/html && composer install && cd wp-content/themes/centralmarket-theme/ && npm ci && npm run prod && echo consider running "docker-command docker exec centralmarket.com wp plugin activate centralmarket-react-service"'
